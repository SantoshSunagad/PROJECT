...
# 12 "./common.h" 2
# 26 "./common.h"
unsigned int SCREEN_FLAG = 0, MENU_FLAG;
unsigned char key;
...
void display_dashboard() {
    ...
    if (g_index == 1 && speed >= 20) {
        speed = 20;
    } else if (g_index == 2 || g_index == 0 || g_index == 8) {
        speed = 0;
    } else if (g_index == 3 && speed >= 30) {
        speed = 30;
    } else if (g_index == 4 && speed >= 40) {
        speed = 40;
    } else if (g_index == 5 && speed >= 60) {
        speed = 60;
    } else if (g_index == 6 && speed >= 80) {
        speed = 80;
    } else if (g_index == 7 && speed >= 99) {
        speed = 99;
    }
    if (write_eeprom) {
        write_eeprom = 0;
    ...
    if (key == 1) {
        ...
        write_eeprom = 1;
        EF = 0;
    } else if (key == 2 && g_index <= 6 || key == 2 && g_index == 8) {
        if (g_index == 8)
            g_index = 2;
        else
            g_index++;
        write_eeprom = 1;
        EF = 0;
    } else if (key == 3 && g_index > 1 || key == 3 && g_index == 8) {
        if (g_index == 8)
            g_index = 2;
        else
            g_index--;
    ...
    if (!EF) {
        clcd_print(gear[g_index], (0xC0 + (10)));
        clcd_putch((speed / 10) + '0', (0xC0 + (14)));
        clcd_putch((speed % 10) + '0', (0xC0 + (15)));
    } else {
        clcd_print(gear[EI], (0xC0 + (10)));
        clcd_putch('0', (0xC0 + (14)));
        clcd_putch('0', (0xC0 + (15)));
    }
}
    ...
    if (chance > 0) {
        ...
        if (index < 4) {
            ...
            if (index == count) {
                clcd_write(0x0C, 0);
                clcd_write(0x01, 0);
                clcd_print("WEL-COME TO ", (0x80 + (0)));
                clcd_print("MENU SCREEN", (0xC0 + (5)));
                ...
                clcd_print(" ATTEMPTS ", (0xC0 + (0)));
                if (chance > 0) {
                    clcd_putch('0' + chance, (0xC0 + (10)));
                } else {
                    clcd_print("OVER", (0xC0 + (10)));
                }
                for (unsigned long y = 0xFFFFA; --y;);
                clcd_write(0x01, 0);
                index = 0;
...
void view_log() {
    ...
    if (delay < 500 && key == 0xFF) {
        if (NEW_KEY == 5 && Event <= no_of_event - 1) {
    ...
    if (no_of_event > 0) {
        ...
        clcd_putch(Event + '0', (0xC0 + (0)));
        clcd_print(print, (0xC0 + (2)));
        clcd_putch((pk / 10) + '0', (0xC0 + (14)));
        clcd_putch((pk % 10) + '0', (0xC0 + (15)));
    } else {
        clcd_print("NO EVENT's", (0xC0 + (3)));
    }
    if (delay >= 500) {
        clcd_write(0x01, 0);
        SCREEN_FLAG = 0;
...
void download_log() {
    ...
    if (no_of_event > 0) {
        ...
        clcd_print("DOWNLOAD COMPLETED", (0x80 + (0)));
        for (unsigned long y = 0xFFFFA; --y;);
    } else {
...
void set_time(unsigned char key) {
    ...
    if (delay < 500 && key == 0xFF) {
        ...
        if (NEW_KEY == 5 && FILED < 3) {
            FILED++;
        } else if (NEW_KEY == 5 && FILED == 3) {
            FILED = 1;
        }
        NEW_KEY = 0;
        delay = 0;
    ...
    if (delay > 500) {
        ...
        if (NEW_KEY == 5) {
            clock_set[0] = read_ds1307(0x02);
            clock_set[1] = read_ds1307(0x01);
            clock_set[2] = read_ds1307(0x00);
            write_ds1307(0x02, ((clock_set[0] & 0x00) | ((hour / 10) << 4) | (hour % 10)));
            write_ds1307(0x01, (((clock_set[1]) & 0x00) | (((min / 10) << 4) | (min % 10))));
            write_ds1307(0x00, ((clock_set[2] & 0x00) | ((second / 10) << 4) | (second % 10)));
            clcd_write(0x01, 0);
...
void change_password() {
    if (index < 4) {
        if (once) {
            once = 0;
            clcd_write(0x01, 0);
            clcd_print(" ENTER NEW PSW ", (0x80 + (0)));
            clcd_write(0xC4, 0);
            clcd_write(0x0F, 0);
            key = 0;
        }
        ...
        if (index == 4 && !password) {
            clcd_write(0x01, 0);
            clcd_print(" ENTER CNF PSW ", (0x80 + (0)));
            clcd_write(0xC4, 0);
            clcd_write(0x0F, 0);
            index = 0;
            count = 0;
            ...
            clcd_write(0x0C, 0);
            clcd_write(0x01, 0);
            clcd_print("PSW CHANGED ", (0x80 + (3)));
            clcd_print("SUCCESSFULLY ", (0x80 + (3)));
            for (unsigned long y = 0xFFFFA; --y;);
            clcd_write(0x01, 0);
            EI = 11;
            EF = 1;
            ...
            clcd_write(0x0C, 0);
            clcd_write(0x01, 0);
            clcd_print("PSW DOES NOT", (0x80 + (3)));
            clcd_print("MATCH", (0xC0 + (5)));
            for (unsigned long y = 0xFFFFA; --y;);
            clcd_write(0x01, 0);
            index = 0;
            count = 0;
...
unsigned int M_index = 0, ARROW = 0;
...
void display_menu() {
    ...
    if (delay < 500 && key == 0xFF) {
        if (M_index < 3 && NEW_KEY == 5) {
            (ARROW) ? (M_index++) : (ARROW = 1);
        }
        if (M_index > 0 && NEW_KEY == 6) {
            (!ARROW) ? (M_index--) : (ARROW = 0);
        } else if (NEW_KEY == 5 && !ARROW) {
            ARROW = 1;
        } else if (NEW_KEY == 6 && ARROW) {
            ARROW = 0;
        }
        delay = 0;
        NEW_KEY = 0;
    ...
    clcd_print(diplay[M_index], (0x80 + (3)));
    clcd_print(diplay[M_index + 1], (0xC0 + (3)));
